...
try:
    from requests_enhanced import OAuth1EnhancedSession, OAuth2EnhancedSession, OAUTH_AVAILABLE
...
def oauth1_twitter_example():
    ...
    if consumer_key == 'your_consumer_key':
        print("⚠️  Set TWITTER_CONSUMER_KEY and TWITTER_CONSUMER_SECRET environment variables")
    ...
    try:
        ...
        verifier = input("Enter the PIN/verifier from Twitter (or press Enter to skip): ").strip()
        ...
        if verifier:
            ...
            if response.status_code == 200:
                ...
                print(f"✅ Authenticated as: @{user_data.get('screen_name', 'unknown')}")
            else:
...
def oauth2_github_example():
    ...
    try:
        ...
        auth_response = input("Enter the full callback URL (or press Enter to skip): ").strip()
        ...
        if auth_response:
            ...
            print(f"✅ Access token received (expires in {token.get('expires_in', 'N/A')} seconds)")
...
def main():
    ...
    print("   🔗 requests-enhanced documentation: https://github.com/khollingworth/requests-enhanced")
