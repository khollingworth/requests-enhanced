...
class Session(requests.Session):
    ...
    def __init__(
        ...
        logger.debug(f"Created Session with timeout={timeout}, retry_config={retry_config}")
    ...
    def request(
        ...
        **kwargs: Any
        ...
        try:
            ...
            raise RequestTimeoutError(f"Request to {url} timed out", original_exception=e)
        except requests.exceptions.RetryError as e:
            # Handle retry exhaustion with our custom error type
            logger.error(f"Max retries exceeded for request to {url}")
            raise RequestRetryError(f"Max retries exceeded for request to {url}", original_exception=e)
